const axios = require('axios'); // ✅ ADD THIS AT TOP (IMPORTANT)
...
class AssessmentController {
  ...
  //       schoolId: student.studentDetails.schoolId, // ✅ FIX
  //       name: student.name,                        // optional but safer
  //       heightInFt,
  //       heightInCm,
  //       weightInKg,
  //       bmi,
  ...
  //     await assessment.populate('studentId', 'name email memberId');
...
//     // 2️⃣ Convert school CODE → ObjectId
//     const school = await School.findOne({
//       schoolId: student.studentDetails.schoolId
//     });
...
//     // 3️⃣ Create assessment (CORRECT TYPES)
//     const assessment = await Assessment.create({
//       studentId: student._id,
//       schoolId: school._id, // ✅ ObjectId
//       name: student.name,
...
// }
  ...
  async getAssessment(req, res) {
    try {
      const assessment = await Assessment.findById(req.params.id)
        .populate('studentId', 'name email dob gender memberId studentDetails')
  ...
  /**
   * Get assessment PDF by ID  Cloudnary
   */
async getAssessmentPdf(req, res) {
  try {
    ...
    if (pdfUrls.length === 0) {
      return res.status(404).json({
        status: 'error',
        message: 'No valid assessment PDFs found'
      });
    }
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=${schoolId}_assessments_${Date.now()}.zip`
    );
    ...
    for (const url of pdfUrls) {
      try {
        ...
        console.warn(`⚠️ Skipping broken PDF: ${url}`);
      }
    }
    ...
    return res.status(500).json({
      status: 'error',
      message: 'Error generating assessment ZIP'
    });
  }
}

  /**
   * Get assessment PDF by ID  AWS
   */
  // async getAssessmentPdf(req, res) {
  //   try {
