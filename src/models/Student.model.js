...
StudentSchema.pre('save', async function (next) {
  ...
  const { schoolId, grade, section } = this.studentDetails;
  if (!schoolId || !grade || !section) {
    return next(new Error('School, Grade, Section are required'));
  }

  /* ---------- SCHOOL (2 digits) ---------- */
  const schools = await this.constructor.distinct('studentDetails.schoolId');
  const schoolIndex = schools.indexOf(schoolId);
  const schoolCode = String(
    schoolIndex === -1 ? schools.length : schoolIndex
  ).padStart(2, '0');

  /* ---------- CLASS (Number OR Letter) ---------- */
  let classCode = '';

  const gradeNumber = grade.match(/\d+/);
  if (gradeNumber) {
    // Grade 1, Grade 10 → 1, 10
    classCode = gradeNumber[0];
  } else {
    // Grade A, Grade B → A, B
    classCode = grade.trim().slice(-1).toUpperCase();
  }

  /* ---------- SECTION (Letter) ---------- */
  const sectionCode = section.trim().toUpperCase();
  ...
  let seq = 0;
  if (lastStudent?.memberId) {
    seq = parseInt(lastStudent.memberId.slice(-2), 10) + 1;
  }

  const studentSeq = String(seq).padStart(2, '0');

  /* ---------- FINAL MEMBER ID ---------- */
  this.memberId = `${schoolCode}${classCode}${sectionCode}${studentSeq}`;
